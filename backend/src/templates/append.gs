...
function fetchMetabaseCardToSheet_functionName(baseUrl, token, cardId, sheetName, numColumns, username, password) {
  ...
  try {
    let response = makeMetabaseRequest_functionName(queryUrl, token);
    ...
    Logger.log(`Appending new data to ${sheetName}...`);
    sheet.getRange(startRow, 1, data.length, numColumns).setValues(data);

    const afterCount = getLastRowForRange_functionName(sheet, numColumns) - 1;
    const expectedAfter = beforeCount + data.length;
    const status = (afterCount === expectedAfter) ? "✅" : "❌";
